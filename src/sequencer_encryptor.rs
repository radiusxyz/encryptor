...
impl SequencerPoseidonEncryption {
  ...
  pub fn get_message_bls_scalar_vector(&self, message_bytes: &[u8]) -> [BlsScalar; SequencerPoseidonEncryption::capacity()] {
    ...
    let mut messages = [BlsScalar::zero(); SequencerPoseidonEncryption::capacity()];
    let mut index = 0;
  ...
  pub fn encrypt(&self, message: String, secret: JubJubAffine) -> (Vec<String>, BlsScalar, [BlsScalar; SequencerPoseidonEncryption::capacity()], [BlsScalar; CIPHER_SIZE]) {
    ...
    let cipher_text_bytes = SequencerPoseidonEncryption::to_bytes(cipher_scalar);
  ...
  pub fn encrypt_scalar(&self, message: &[BlsScalar], secret: &JubJubAffine) -> ([BlsScalar; CIPHER_SIZE], BlsScalar) {
    ...
    let mut state = SequencerPoseidonEncryption::initial_state(&secret, nonce);
  ...
  pub fn decrypt(&self, cipher_text_hexes: Vec<String>, secret: &JubJubAffine, nonce: String) -> Vec<u8> {
    ...
    for (_, cipher_text_hex) in cipher_text_hexes.iter().enumerate() {
      ...
      let cipher_text = SequencerPoseidonEncryption::from_bytes(&chipher_text_hex_bytes).unwrap();
      ...
      if cipher_text[MESSAGE_CAPACITY] != state[1] {
        return SequencerPoseidonEncryption::convert_bls_scalar_to_message(Vec::new());
      }

      result.extend_from_slice(&message);
    }

    SequencerPoseidonEncryption::convert_bls_scalar_to_message(result)
  }
